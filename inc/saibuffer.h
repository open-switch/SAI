...
typedef enum _sai_ingress_priority_group_attr_t
{
     ...
     *
     * @note This PG OID is used to get PG counter per XPE.
     * @par Each PG can belongs to one or more XPE's in NPU.
     * @par OID is valid only at index where XPE is valid for that PG else it is NULL.
     * @par Example:
     *   NPU can have 4 XPE's (0,1,2,3). If PG belongs to XPE1, XPE2, the list is:
     *   @li Index 0 - SAI_OBJECT_ID_NULL
     *   @li Index 1 - Shadow PG OID-1
     *   @li Index 2 - Shadow PG OID-2
     *   @li Index 3 - SAI_OBJECT_ID_NULL
     *
...
typedef enum _sai_ingress_priority_group_stat_t
{
    ...
    /** Get current pg occupancy from BST snapshot in bytes [uint64_t] */
    SAI_INGRESS_PRIORITY_GROUP_STAT_EXTENSIONS_SNAPSHOT_CURR_OCCUPANCY_BYTES = SAI_INGRESS_PRIORITY_GROUP_STAT_CUSTOM_RANGE_BASE,

    /** Get watermark pg occupancy from BST snapshot in bytes [uint64_t] */
    SAI_INGRESS_PRIORITY_GROUP_STAT_EXTENSIONS_SNAPSHOT_WATERMARK_BYTES,

    /** Get current pg shared occupancy from BST snapshot in bytes [uint64_t] */
    SAI_INGRESS_PRIORITY_GROUP_STAT_EXTENSIONS_SNAPSHOT_SHARED_CURR_OCCUPANCY_BYTES,

    /** Get watermark pg shared occupancy from BST snapshot in bytes [uint64_t] */
    SAI_INGRESS_PRIORITY_GROUP_STAT_EXTENSIONS_SNAPSHOT_SHARED_WATERMARK_BYTES,

    /** Get current pg XOFF room occupancy from BST snapshot in bytes [uint64_t] */
    SAI_INGRESS_PRIORITY_GROUP_STAT_EXTENSIONS_SNAPSHOT_XOFF_ROOM_CURR_OCCUPANCY_BYTES,

    /** Get watermark pg XOFF room occupancy from BST snapshot in bytes [uint64_t] */
...
typedef enum _sai_buffer_pool_stat_t
{
    ...
    /** Get current pool occupancy from BST snapshot in bytes [uint64_t] */
    SAI_BUFFER_POOL_STAT_EXTENSIONS_SNAPSHOT_CURR_OCCUPANCY_BYTES = SAI_BUFFER_POOL_STAT_CUSTOM_RANGE_BASE,

    /** Get watermark pool occupancy from BST snapshot in bytes [uint64_t] */
    SAI_BUFFER_POOL_STAT_EXTENSIONS_SNAPSHOT_WATERMARK_BYTES,

    /** Get current Shared headroom pool occupancy from BST snapshot in bytes [uint64_t] */
    SAI_BUFFER_POOL_STAT_EXTENSIONS_SNAPSHOT_XOFF_ROOM_CURR_OCCUPANCY_BYTES,

    /** Get watermark Shared headroom pool occupancy from BST snapshot in bytes [uint64_t] */
