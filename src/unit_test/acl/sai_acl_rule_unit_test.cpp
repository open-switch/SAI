...
TEST_F(saiACLRuleTest, rule_with_drop_mark)
{
    sai_status_t    sai_rc = SAI_STATUS_SUCCESS;
    sai_object_id_t acl_rule_id = 0;
    sai_object_id_t acl_table_id = 0;
    ...
    sai_rc = sai_test_acl_table_create (&acl_table_id, 10,
                             ...
                             SAI_ACL_TABLE_ATTR_FIELD_DSCP,
                             SAI_ACL_TABLE_ATTR_FIELD_IP_PROTOCOL,
                             SAI_ACL_TABLE_ATTR_FIELD_IPV6_NEXT_HEADER, SAI_ACL_TABLE_ATTR_FIELD_DROP_MARKED);
    EXPECT_EQ (SAI_STATUS_SUCCESS, sai_rc);
    ...
    sai_rc = sai_test_acl_rule_create (&acl_rule_id, 6,
                                ...
                                SAI_ACL_ENTRY_ATTR_FIELD_IPV6_NEXT_HEADER,
                                1, 6, 0xff,
                                SAI_ACL_ENTRY_ATTR_FIELD_DROP_MARKED, true, 1);
...
TEST_F(saiACLRuleTest, rule_with_bridge_type_match)
{
    sai_status_t    sai_rc = SAI_STATUS_SUCCESS;
    sai_object_id_t acl_rule_id = 0;
    sai_object_id_t acl_table_id = 0;
    ...
    /* Table Create */
    sai_rc = sai_test_acl_table_create (&acl_table_id, 20,
                                        SAI_ACL_TABLE_ATTR_ACL_STAGE,
                                        SAI_ACL_STAGE_INGRESS,
                                        SAI_ACL_TABLE_ATTR_PRIORITY, 10,
                                        SAI_ACL_TABLE_ATTR_FIELD_DST_IPV6,
    ...
    /*Create IGMP rule which will be applied only for .1D bridges*/
    sai_rc = sai_test_acl_rule_create (&acl_rule_id, 5,
                                       SAI_ACL_ENTRY_ATTR_TABLE_ID, acl_table_id,
                                       SAI_ACL_ENTRY_ATTR_PRIORITY, 5,
                                       SAI_ACL_ENTRY_ATTR_ADMIN_STATE, true,
                                       ...
                                       1, SAI_BRIDGE_TYPE_1D, standard_mask);

    /*Verify the bridge type attribute value*/
    sai_rc = sai_test_acl_rule_get (acl_rule_id, &acl_entry_attr,
                                    1, SAI_ACL_ENTRY_ATTR_FIELD_BRIDGE_TYPE);
    EXPECT_EQ (SAI_STATUS_SUCCESS, sai_rc);
    EXPECT_EQ (acl_entry_attr.value.aclfield.enable, true);
    EXPECT_EQ (acl_entry_attr.value.aclfield.data.s32, SAI_BRIDGE_TYPE_1D);

    /*Modify the IGMP rule so that they are applied only to .1Q briges*/
    sai_rc = sai_test_acl_rule_set (acl_rule_id, 1, SAI_ACL_ENTRY_ATTR_FIELD_BRIDGE_TYPE,
                                    true, SAI_BRIDGE_TYPE_1Q, standard_mask);
    EXPECT_EQ (SAI_STATUS_SUCCESS, sai_rc);
    ...
    EXPECT_EQ (SAI_STATUS_SUCCESS, sai_rc);
    EXPECT_EQ (acl_entry_attr.value.aclfield.enable, true);
    EXPECT_EQ (acl_entry_attr.value.aclfield.data.s32, SAI_BRIDGE_TYPE_1Q);

    /*Remove the bridge type field so that IGMP rule is applied to both 1Q and
     *1D bridges*/
    sai_rc = sai_test_acl_rule_set (acl_rule_id, 1, SAI_ACL_ENTRY_ATTR_FIELD_BRIDGE_TYPE,
                                    false, 0, 0);
    EXPECT_EQ (SAI_STATUS_SUCCESS, sai_rc);

    /*Verify the bridge type field attribute is disabled*/
    sai_rc = sai_test_acl_rule_get (acl_rule_id, &acl_entry_attr,
                                    1, SAI_ACL_ENTRY_ATTR_FIELD_BRIDGE_TYPE);
    EXPECT_EQ (SAI_STATUS_SUCCESS, sai_rc);
    EXPECT_EQ (acl_entry_attr.value.aclfield.enable, false);
